...
function App() {
  ...
  const checkMimeType = event => {
    ...
    const types = ['image/png', 'image/jpeg', 'image/gif', 'application/pdf', 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document']
    // loop access array
    for(let x = 0; x<files.length; x++) {
      // compare file type find doesn't match
      if(types.every(type => files[x].type !== type)) {
        // create error massage and assign to container
        err[x] = files[x].type + ' is not supported format or files do not match'
      }
  };
